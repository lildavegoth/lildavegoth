...
module.exports = async (req, res) => {
    ...
    if (action === 'login') {
        ...
        global.verifiers = global.verifiers || {};
        global.verifiers[session] = { verifier, expires: Date.now() + 600000 };
        ...
        const tokenReq = https.request({
            ...
            tokenRes.on('end', () => {
                try {
                    ...
                    global.tokens[state] = { tokens, expires: Date.now() + 300000 };
    ...
    if (action === 'tokens') {
        ...
        global.tokens = global.tokens || {};
        const entry = global.tokens[session];
        if (!entry || Date.now() > entry.expires) return res.json({ ready: false });
        delete global.tokens[session];
